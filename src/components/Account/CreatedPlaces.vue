<template>
  <div v-if="dataLoaded">
    ...
    <h2>{{ $t('account.created_places') }}</h2>
    <div class="mt-4">
      <div v-if="items.length > 0">
        <router-link v-for="place in items" :key="place.title"
                     :to="{name: 'PlaceDetails', params: {placeTitle: place.title}}" class="asp-link m-1">
          {{ place.title }}
        </router-link>
      ...
      <div v-else>{{ $t('empty_result.place.created') }}</div>
    </div>
  </div>
</template>
  ...
  methods: {
    mapInvalidResponse(result) {
      let errorCode = 'authentication' in result.codes
          ? 'authentication'
          : 'created_places' in result.codes
              ? 'created_places' : '';
    ...
    async getData(withOffset = true) {
      let response;
      try {
        let url = '/api/user/account/places';
        ...
        return {
          codes: { 'created_places': errors.routes.get.details.account.created_places },
          status: status.error
        };
      }
